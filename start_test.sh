...
logit "INFO" "Waiting for all slave pods to be terminated before recreating the pod set"
...
if [ -z "${nb_injectors}" ]; then
    logit "WARN" "Keeping number of injector to 1"
    ...
    logit "INFO" "Finished scaling the number of pods."
fi
...
if [ -n "${module}" ]; then
    ...
    logit "INFO" "Finished copying modules to slave pods"
fi

logit "INFO" "Copying ${jmx} to slave pods"
logit "INFO" "Number of slaves is ${slave_num}"
...
logit "INFO" "Finished copying scenario to slave pods"
...
kubectl port-forward $(kubectl get pod | grep grafana |awk '{print $1}') 3000 -n ${namespace} &
logit "INFO" "##################################################"
...
logit "WARN" "DO NOT SHARE THESE CREDENTIALS"
